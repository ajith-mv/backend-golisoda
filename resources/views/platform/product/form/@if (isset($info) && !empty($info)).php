@if (isset($info) && !empty($info))
    ...
    {{-- @foreach ($info->productAttributes as $attr) --}}
        <div class="form-group d-flex flex-wrap gap-5 childRow">
                ...
                <select class="form-select product-attr-select required" name="filter_variation[]"
                    ...
                    @isset($variations)
                                @foreach ($variations as $item)
                                    <option value="{{ $item->id }}" @if(isset($info->variation_id) && $info->variation_id == $item->id) selected="selected" @endif>
                                        {{ $item->title }} 
                                    </option>
                                @endforeach
                            @endisset
            ...
            <!--end::Input-->
            <input type="checkbox" name="is_overview_" value="1"
...
@else
    <div class="form-group d-flex flex-wrap gap-5 childRow">
        ...
        <div class="w-100 w-md-300px">
            <select class="form-select product-attr-select required" name="variation"
                aria-label="Select a Variation" data-control="select2" data-placeholder="Select a variation...">
                <option></option>
                @foreach ($variations as $item)
        ...
        <!--end::Select2-->
        <div class="w-100 w-md-300px" id="variation_data">
            <select class="form-select product-attr-select required" name="variation_value" id="variation_value"
                aria-label="Choose variation value" data-control="select2" data-placeholder="Choose variation value">
                <option></option>
            </select>
        </div>
        
        <input type="text" class="form-control mw-100 w-100px required numberonly"
            name="filter_variation_order[]" value=""  placeholder="Amount" required />
        <!--end::Input-->
